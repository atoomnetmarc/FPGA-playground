...
module top_module (
  ...
  reg [3:0] q = 4'b0;
  reg [15:0] largecounter = 16'b0;
  reg shift_ena;

  assign count = (largecounter / 1000);

  //Determine next state.
  always @(*) begin
    casez ({
      data, (largecounter == 4'b0), ack, state
    })
  ...
  always @(posedge clk) begin
    ...
    if (shift_ena) begin
      q <= {q[2:0], data};
      largecounter = (({q[2:0], data} + 1'b1) * 12'd1000) - 1'b1;
    end
    if (counting & largecounter > 1'b0) begin
      largecounter <= largecounter - 1'b1;
    end
  end
