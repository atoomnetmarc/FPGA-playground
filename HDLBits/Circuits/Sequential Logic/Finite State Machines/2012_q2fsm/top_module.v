...
module top_module (
  ...
  always @(*) begin
    case ({
      ...
      {1'b0, `STATE_B} : next_state = `STATE_D;
      {1'b1, `STATE_B} : next_state = `STATE_C;
      {1'b0, `STATE_C} : next_state = `STATE_D;
      {1'b1, `STATE_C} : next_state = `STATE_E;
      {1'b0, `STATE_D} : next_state = `STATE_A;
      {1'b1, `STATE_D} : next_state = `STATE_F;
      {1'b0, `STATE_E} : next_state = `STATE_D;
      {1'b1, `STATE_E} : next_state = `STATE_E;
      {1'b0, `STATE_F} : next_state = `STATE_D;
      {1'b1, `STATE_F} : next_state = `STATE_C;
